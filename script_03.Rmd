...
```{r sc1}
sep_new <- read_excel("new_data/new_data.xlsx", 
             skip = 3) %>% 
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
...
- define precisely what we want to know
- in this case, we want to know the difference between the new and old course enrollment numbers.




After we have a model, Temperance suggests we should:
- Acknowledge and remember that our model has limitations.
- In this case, the pandemic may force students in or out of enrollment
...
```{r}
difference <- inner_join(sep_old, sep_new,
                         by = c("id", "title", "name", "department")) %>% 
  rename(old_u_grad = u_grad.x,new_u_grad = u_grad.y )
```
